    ...
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1 justify-end" style={{ backgroundColor: theme.scrim }}>
        <View className="h-[94%] rounded-t-[28px] bg-background p-4">
          ...
          <View className="my-3 flex-row items-center gap-1.5"><Action label={t('pane.splitRight')} icon="return-down-forward-outline" onPress={() => run(() => client.splitPane(pane.pane_id, 'right'))} /><Action label={t('pane.splitDown')} icon="return-down-back-outline" onPress={() => run(() => client.splitPane(pane.pane_id, 'down'))} /><IconButton icon="expand-outline" accessibilityLabel={t('pane.zoom')} onPress={() => run(() => client.zoomPane(pane.pane_id))} /><Button size="sm" onPress={hapticPress(() => onOpenTerminal(pane))}><Ionicons name="terminal-outline" size={16} color={theme.onPrimary} /><Text>{t('pane.open')}</Text></Button></View>
          <View className="min-h-9 flex-row items-center justify-between"><Text className="text-sm font-semibold">{t('pane.liveOutput')}</Text><IconButton icon="refresh" accessibilityLabel={t('pane.refreshOutput')} className="size-[34px]" onPress={read} /></View>
          ...
          <View className="mt-2.5 min-h-[52px] flex-row items-center rounded-full border border-border bg-card py-1 pl-3.5 pr-1.5"><Input className="h-10 flex-1 border-0 bg-transparent px-0 shadow-none" value={command} onChangeText={setCommand} placeholder={t('pane.sendPlaceholder')} /><IconButton icon="arrow-up" accessibilityLabel={t('pane.send')} disabled={!command.trim() || busy} selected={Boolean(command.trim())} onPress={() => run(async () => { await client.runInPane(pane.pane_id, command); setCommand(''); })} /></View>
          <Button className="mt-2 rounded-full" variant="destructive" onPress={hapticPress(() => Alert.alert(t('pane.closeTitle'), pane.label || pane.pane_id, [{ text: t('common.cancel'), style: 'cancel' }, { text: t('common.close'), style: 'destructive', onPress: () => run(() => client.closePane(pane.pane_id), true) }]))}><Ionicons name="trash-outline" size={17} color="#FFFFFF" /><Text>{t('pane.closePane')}</Text></Button>
        </View>
