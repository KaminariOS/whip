...
export function LiveSessionRail({ sessions, activeHostId, onSelect, onClose, onNew }: Props) {
  ...
  return (
    <View className="h-12 flex-row items-stretch border-b border-terminal-divider bg-terminal-panel">
...
function HostPill({ session, active, onSelect, onClose }: { session: LiveSessionRailItem; active: boolean; onSelect: () => void; onClose?: () => void }) {
  const { t } = useTranslation();
  return (
    <View className={cn('h-8 max-w-[190px] flex-row items-center overflow-hidden rounded-full bg-terminal-surface', active && 'bg-terminal-accent')}>
      <Button accessibilityLabel={t(session.hostId ? 'rail.openHost' : 'rail.showHosts', { host: session.label, status: session.agentStatus })} accessibilityRole="radio" accessibilityState={{ selected: active }} className="h-8 min-w-0 flex-shrink justify-start gap-1.5 rounded-none px-2.5 py-0" variant="ghost" onPress={hapticPress(onSelect)}>
        ...
        <Text className={cn('max-w-[119px] pb-0.5 text-[11px] font-semibold leading-[18px] text-terminal-text', active && 'text-terminal-ink')} numberOfLines={1}>{session.label}</Text>
        {session.terminalCount > 0 ? <Text className={cn('text-[10px] leading-[18px] text-terminal-subtle', active && 'text-terminal-ink')}>{session.terminalCount}</Text> : null}
      </Button>
