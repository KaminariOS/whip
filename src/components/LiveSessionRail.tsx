...
function HostPill({ session, active, onSelect, onClose }: { session: LiveSessionRailItem; active: boolean; onSelect: () => void; onClose?: () => void }) {
  return (
    <View className={cn('h-8 max-w-[190px] flex-row items-center overflow-hidden rounded-full bg-[#2F2F2F]', active && 'bg-[#ECECEC]')}>
      <Button accessibilityLabel={`${session.hostId ? 'Open' : 'Show'} ${session.label}, ${session.agentStatus}`} accessibilityRole="radio" accessibilityState={{ selected: active }} className="h-8 min-w-0 flex-shrink justify-start gap-1.5 rounded-none px-2.5 py-0" variant="ghost" onPress={hapticPress(onSelect)}>
      ...
      {onClose ? <Button accessibilityLabel={`Disconnect ${session.label}`} className="h-8 w-7 rounded-none px-0" variant="ghost" onPress={hapticPress(onClose)}><Ionicons name="close" size={14} color={active ? colors.ink : colors.muted} /></Button> : null}
    </View>
  );
}
