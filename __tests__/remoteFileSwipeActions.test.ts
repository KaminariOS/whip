...
describe('remote file swipe actions', () => {
  ...
  it('clamps the row to the delete action width', () => {
    ...
    expect(remoteFileSwipeOffset(40, true)).toBe(
      -REMOTE_FILE_SWIPE_ACTION_WIDTH + 40,
    );
  ...
  it('settles using drag distance or velocity', () => {
    expect(
      shouldOpenRemoteFileSwipe(
        -REMOTE_FILE_SWIPE_ACTION_WIDTH * 0.6,
        0,
        false,
      ),
    ).toBe(true);
    ...
    expect(
      shouldOpenRemoteFileSwipe(REMOTE_FILE_SWIPE_ACTION_WIDTH, 0, true),
    ).toBe(false);
  });
});
