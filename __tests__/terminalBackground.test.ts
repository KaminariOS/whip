...
test('copies selected images into the backed-up terminal background directory', async () => {
  ...
  await expect(selectTerminalBackgroundImage(previousUri)).resolves.toBe(
    'file:///data/user/0/io.github.kaminarios.whip/files/terminal-backgrounds/herdr-terminal-background-1234.png',
  );
...
test('moves a legacy app-private image into the backed-up directory', async () => {
  const legacyUri = 'file:///data/user/0/io.github.kaminarios.whip/files/herdr-terminal-background-1.webp';
  mockExistingFiles.add(legacyUri);

  await expect(migrateTerminalBackgroundImage(legacyUri)).resolves.toBe(
    'file:///data/user/0/io.github.kaminarios.whip/files/terminal-backgrounds/herdr-terminal-background-1.webp',
  );
  expect(mockCopies[0]).toEqual({
    source: legacyUri,
    destination: 'file:///data/user/0/io.github.kaminarios.whip/files/terminal-backgrounds/herdr-terminal-background-1.webp',
    overwrite: true,
  });
});

test('rebases a restored image URI to the current Android app sandbox', async () => {
  const currentUri = 'file:///data/user/0/io.github.kaminarios.whip/files/terminal-backgrounds/herdr-terminal-background-1.webp';
  mockExistingFiles.add(currentUri);

  await expect(migrateTerminalBackgroundImage(
    'file:///data/user/10/io.github.kaminarios.whip/files/terminal-backgrounds/herdr-terminal-background-1.webp',
