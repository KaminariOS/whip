...
test_dir="$(mktemp -d)"
container_name="whip-rust-ssh-${RANDOM}-$$"
image="${WHIP_SSH_TEST_IMAGE:-lscr.io/linuxserver/openssh-server@sha256:4e3054a3c64f19cf4ee28dcac64c030f3a722a4fe46a319f68f9e0952c7de074}"
...
ssh-keygen -q -t ed25519 -N '' -C whip-live-test -f "$test_dir/client_key"
printf '%s\n' \
...
"$container_runtime" run --detach --name "$container_name" \
  ...
  --env PGID="$(id -g)" \
  --env TZ=Etc/UTC \
  --env PUBLIC_KEY_FILE=/run/secrets/whip_test_key.pub \
  --env PASSWORD_ACCESS=true \
  --env USER_PASSWORD=whip-test-password \
  --env USER_NAME=whip \
  --env LOG_STDOUT=true \
  --publish 127.0.0.1::2222 \
  --volume "$test_dir/client_key.pub:/run/secrets/whip_test_key.pub:ro" \
...
(
  cd "$rust_dir"
  WHIP_SSH_TEST_HOST=127.0.0.1 \
  WHIP_SSH_TEST_PORT="$host_port" \
  WHIP_SSH_TEST_TARGET_PORT=2222 \
  WHIP_SSH_TEST_USER=whip \
  WHIP_SSH_TEST_PRIVATE_KEY="$test_dir/client_key" \
  WHIP_SSH_TEST_KNOWN_HOSTS="$test_dir/known_hosts" \
  WHIP_SSH_TEST_SHARED_DIR="$test_dir/shared" \
