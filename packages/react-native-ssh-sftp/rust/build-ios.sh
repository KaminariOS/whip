...
if ! command -v rustup >/dev/null 2>&1; then
  echo "error: rustup is required to build the Whip iOS SSH transport" >&2
  exit 1
fi
...
for architecture in $architectures; do
  ...
  libraries+=("$rust_dir/target/$target/release/libwhip_ssh.a")
done

if [[ ${#libraries[@]} -eq 1 ]]; then
  cp "${libraries[0]}" "$build_dir/libwhip_ssh.a"
else
  xcrun lipo -create "${libraries[@]}" -output "$build_dir/libwhip_ssh.a"
fi
