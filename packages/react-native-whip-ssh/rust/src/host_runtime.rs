...
async fn runtime_remote_home(inner: &RuntimeInner) -> Result<String, HostRuntimeError> {
    let generation = current_generation(inner)?;
    let home = current_ssh(inner)?.remote_home().await?;
    validate_generation(inner, generation)?;
    crate::remote_ops::normalize_absolute(home.trim()).map_err(HostRuntimeError::RemoteFileFailure)
}

async fn resolved_remote_path(
    inner: &RuntimeInner,
    path: Option<&str>,
) -> Result<String, HostRuntimeError> {
    let home = runtime_remote_home(inner).await?;
...
fn invalidate_remote_operations(inner: &Arc<RuntimeInner>, generation: u64, reason: &str) {
...
async fn start_path_preview_inner(
    inner: Arc<RuntimeInner>,
    remote_path: String,
    html: bool,
...
async fn transfer_setup_step<T, E, F>(
...
async fn execute_generation_checked(
    inner: &RuntimeInner,
    command: &str,
...
fn command_failure(kind: &str, output: &crate::ssh::CommandOutput) -> String {
